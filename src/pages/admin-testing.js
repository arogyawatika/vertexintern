...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        if (savedKey) {
            ...
            setIsLoggedIn(true);
            fetchData('test_certificates', savedKey);
    ...
    const handleLogin = (e) => {
        ...
        setIsLoggedIn(true);
        fetchData('test_certificates', masterKey);
    ...
    const fetchData = async (table, key) => {
        ...
        try {
            ...
            if (res.ok) {
                if (table === 'test_certificates') setCertificates(result.data);
    ...
    const submitCertificate = async (e) => {
        ...
        try {
            const endpoint = editingId ? '/api/admin-action' : '/api/add-test-certificate';
            const payload = editingId 
                ? { masterKey, action: 'update', table: 'test_certificates', id: editingId, payload: certData }
                : { masterKey, certData };
                                            ...
                                            {activeTab === 'certificates' && certificates.map(cert => (
                                                <tr key={cert.id}>
                                                    ...
                                                    <td data-label="Actions" className="action-cells">
                                                        ...
                                                        <button className="btn-icon delete" onClick={() => handleDelete('test_certificates', cert.id)} title="Delete"><IoTrashOutline /></button>
                                                    </td>
                                                </tr>
                                            ))}
